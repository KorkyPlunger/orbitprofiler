...
struct UdtClassInfo 
{
    ...
    void serialize(Archive & ar, std::uint32_t const version)
    {
        ar( //CEREAL_NVP(Name)
          ...
          , CEREAL_NVP(Functions)
          , CEREAL_NVP(BaseClasses));
    }
};
...
struct FunctionTypeInfo 
{
    ...
    void serialize(Archive & ar, std::uint32_t const version)
    {
        ar( CEREAL_NVP(RetTypeIndex)
          , CEREAL_NVP(Args)
          , CEREAL_NVP(CallConv)
          , CEREAL_NVP(MemberFunction)
          , CEREAL_NVP(ClassIndex)
          , CEREAL_NVP(ThisAdjust)
          , CEREAL_NVP(StaticFunction) );
    }
}; 
...
struct TypeInfo 
{
    ...
    void serialize(Archive & ar, std::uint32_t const version)
    {
        ar( CEREAL_NVP(Tag)
          , CEREAL_NVP(UdtKind)
          , CEREAL_NVP(Info)
          , CEREAL_NVP(sUdtClassInfo)
          , CEREAL_NVP(sUdtUnionInfo)
          , CEREAL_NVP(sEnumInfo)
          , CEREAL_NVP(sFunctionTypeInfo)
          );
    }
};
...
//-----------------------------------------------------------------------------
template<class Archive>
void serialize( Archive & archive, TypeInfo::TypeInfoStructures & a_TypeInfoStructures, std::uint32_t const version )
{
