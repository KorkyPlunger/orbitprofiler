...
{
    // Allocate
    LPVOID targetBaseAddress = VirtualAllocEx(m_InjectedProcessHandle, (LPVOID)0, (size_t)a_NumBytes, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
    ...
    // Write
    if( !WriteProcessMemory( m_InjectedProcessHandle, targetBaseAddress, (LPCVOID)a_Data, (size_t)a_NumBytes, 0 ) )
    {
...
bool Injection::Inject( const wstring & a_DllName, const Process & a_Process, const string & ProcName, const std::wstring& captureHost, int capturePort)
{
    ...
    m_InjectedProcessHandle = OpenProcess( PROCESS_ALL_ACCESS, FALSE, m_InjectedProcessID );
    ...
    HMODULE m_hKern32 = GetRemoteModuleHandle(m_InjectedProcessHandle, ("kernel32.dll"));
    if (!m_hKern32)
    {
    ...
    PVOID m_LoadLibraryAddress = (PVOID)GetRemoteProcAddress(m_InjectedProcessHandle, m_hKern32, "LoadLibraryW");
    ...
    // Inject dll in targeted process by creating a thread that calls LoadLibraryW with our dll name
    if( !CreateRemoteThread( m_InjectedProcessHandle, 0, 0, (LPTHREAD_START_ROUTINE)m_LoadLibraryAddress, targetBaseAddress, 0, 0 ) )
    {
    ...
    {
        remoteModuleHandle = GetRemoteModuleHandle( m_InjectedProcessHandle, dllName.c_str() );
        Sleep(50);
    }
    ...
    if (!CreateRemoteThread(m_InjectedProcessHandle, 0, 0, (LPTHREAD_START_ROUTINE)procAdd, hostStringAddress, 0, 0))
    {
...
HANDLE Injection::GetTargetProcessHandle( const string & a_Target, DWORD & o_ProcessID )
{
    ...
    if (Process32First(snapshot, &processinfo))
    {
        do
        {
            ...
            //if (/*TODO:*/ (fn_Wow64SuspendThread == NULL || fn_Wow64GetThreadContext == NULL) && !Is64BitProcess(process_handle)) {
            //    CloseHandle(process_handle);
            //    continue;
            //}
#endif
...
//-----------------------------------------------------------------------------
HMODULE WINAPI Injection::GetRemoteModuleHandle( HANDLE hProcess, LPCSTR lpModuleName )
{
...
//-----------------------------------------------------------------------------
FARPROC WINAPI Injection::GetRemoteProcAddress(HANDLE hProcess, HMODULE hModule, LPCSTR lpProcName, UINT Ordinal, BOOL UseOrdinal)
{
    // https://www.codeproject.com/tips/139349/getting-the-address-of-a-function-in-a-dll-loaded
...
#else
Injection::Injection(){}
bool Injection::Inject( const std::wstring & a_Dll, const Process & a_Process, const std::string & ProcName, const std::wstring& captureHost, int capturePort){return false;}
