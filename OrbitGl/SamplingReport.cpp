...
void SamplingReport::OnSelectAddress( DWORD64 a_Address, ThreadID a_ThreadId )
{
    if( m_CallstackDataView )
    {
        ...
        {
            m_SelectedSortedCallstackReport = m_Profiler->GetSortedCallstacksFromAddress( a_Address, a_ThreadId );
...
wstring SamplingReport::GetSelectedCallstackString()
{
    ...
    {
        int numOccurances = m_SelectedSortedCallstackReport->m_CallStacks[m_SelectedAddressCallstackIndex].m_Count;
        int totalCallstacks = m_SelectedSortedCallstackReport->m_NumCallStacksTotal;

        return Format( L"%i of %i unique callstacks.  [%i/%i total callstacks](%.2f%%)"
                     , m_SelectedAddressCallstackIndex+1
                     , m_SelectedSortedCallstackReport->m_CallStacks.size()
                     , numOccurances
                     , totalCallstacks
                     , 100.f*(float)numOccurances/(float)totalCallstacks );
    }
...
void SamplingReport::OnCallstackIndexChanged( int a_Index )
{
    if( a_Index >= 0 && a_Index < m_SelectedSortedCallstackReport->m_CallStacks.size() )
    {
        ...
        m_CallstackDataView->SetCallStack( m_Profiler->GetCallStack( cs.m_CallstackId ) );
    }
    else
    {
        m_SelectedAddressCallstackIndex = 0;
