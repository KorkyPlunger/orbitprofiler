...
//-----------------------------------------------------------------------------
TextRenderer::TextRenderer() : m_Atlas(NULL)
                             , m_Buffer(NULL)
                             , m_Font(NULL)
                             , m_Canvas(NULL)
                             , m_Initialized(false)
...
void TextRenderer::Display()
{
    ...
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RED, (GLsizei)m_Atlas->width, (GLsizei)m_Atlas->height, 0, GL_RED, GL_UNSIGNED_BYTE, m_Atlas->data );
    ...
    mat4_set_orthographic( &m_Proj, 0, (float)m_Canvas->getWidth(), 0, (float)m_Canvas->getHeight(), -1, 1);
...
//-----------------------------------------------------------------------------
void TextRenderer::AddTextInternal( texture_font_t* font
                                  , const char* text
                                  , const vec4 & color
                                  , vec2 * pen
                                  , float a_MaxSize
                                  ...
                                  , bool a_Static )
{
    ...
    for( i = 0; i < strlen(text); ++i )
    {
        ...
        if (glyph != NULL)
        {
            ...
            vertex_t vertices[4] = { { (float)x0, (float)y0, textZ, s0, t0, r, g, b, a },
...
//-----------------------------------------------------------------------------
void TextRenderer::AddText( const char* a_Text
                          , float a_X
                          , float a_Y
                          , float a_Z
                          , const Color& a_Color
...
void TextRenderer::GetStringSize( const char* a_Text, int & a_Width, int & a_Height )
{
    ...
    for( size_t i = 0; i < len; ++i )
    {
        ...
        if( glyph != NULL )
        {
            ...
            if( glyph->height > stringHeight )
                stringHeight = (float)glyph->height;
        }
    }
