...
QModelIndex OrbitTreeModel::index(int row, int column, const QModelIndex &parent)
            const
{
...
QModelIndex OrbitTreeModel::parent(const QModelIndex &index) const
{
    ...
    OrbitTreeItem *childItem = static_cast<OrbitTreeItem*>(index.internalPointer());
...
void OrbitTreeModel::setupModelData(const QStringList &lines, OrbitTreeItem *parent)
{
    ...
    while (number < lines.count()) {
        ...
        if (!lineData.isEmpty()) {
            ...
            parents.last()->appendChild(new OrbitTreeItem(columnData, parents.last()));
        }
