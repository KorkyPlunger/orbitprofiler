...
void OrbitWatchWidget::SetupPropertyBrowser()
{
    ...
    m_Editor->setFactoryForManager( sizeManager->subIntPropertyManager(), spinBoxFactory );
    m_Editor->setFactoryForManager( rectManager->subIntPropertyManager(), spinBoxFactory );
    m_Editor->setFactoryForManager( sizePolicyManager->subIntPropertyManager(), sliderFactory);
    m_Editor->setFactoryForManager( sizePolicyManager->subEnumPropertyManager(), comboBoxFactory );
    ...
    GOrbitApp->AddUpdateWatchCallback( [this]( const Variable* a_Variable ) { this->OnUpdateWatch( a_Variable ); } );
}

//-----------------------------------------------------------------------------
OrbitWatchWidget::OrbitWatchWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::OrbitWatchWidget)
{
    ui->setupUi(this);
...
//-----------------------------------------------------------------------------
OrbitWatchWidget::~OrbitWatchWidget()
{
    delete ui;
}
...
//-----------------------------------------------------------------------------
QtAbstractPropertyManager* OrbitWatchWidget::GetManager( const Variable* a_Variable )
{
...
QtProperty* OrbitWatchWidget::AddProp( QtProperty* a_Parent, const Variable* a_Variable )
{
    ...
    {
        newProperty = manager->addProperty( QString::fromStdWString( a_Variable->m_Name ) );
        newProperty->setPropertyType( QString::fromStdWString( typeName ) );
...
//-----------------------------------------------------------------------------
void OrbitWatchWidget::on_RefreshButton_clicked()
{
    GOrbitApp->RefreshWatch();
}
