...
{
    ui->setupUi(this);
    m_TreeModel = new OrbitTreeModel(QString("one\n two\n  three\n  four\n five\n"));
    ...
    ui->plainTextEdit->setPlainText("// Copy Visual Studio's output generated when\n// \"Show Includes\" (/showincludes) is active\n// to see a proper tree view on the right pane.");
...
void ShowIncludesDialog::onCustomContextMenu(const QPoint &point)
{
    ...
    if (m_ModelIndex.isValid())
    {
        if( GContextMenuShowIncludes.size() > 0 )
        {
            ...
            {
                actions.push_back( new QAction( QString::fromStdWString( GContextMenuShowIncludes[i] ) ) );
...
ShowIncludesDialog::~ShowIncludesDialog()
{
    delete ui;
}
...
void ShowIncludesDialog::on_pushButton_clicked()
{
...
void ShowIncludesDialog::on_lineEdit_2_textChanged(const QString &arg1)
{
    ...
    iterate( ui->treeView->rootIndex(), ui->treeView->model(), [=]( const QModelIndex & idx, int /*depth*/ )
    {
        OrbitTreeItem *item = static_cast<OrbitTreeItem*>(idx.internalPointer());
        if( item->IsVisible() )
        {
