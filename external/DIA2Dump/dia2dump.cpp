    ...
    IDiaSymbol      **ppGlobal)
{
    ...
    if( FAILED( hr ) )
    {
        ...
        pfnGetFactory procAddr = (pfnGetFactory)GetProcAddress( msDiaModule, "DllGetClassObject" );
        if( procAddr == NULL )
            return false;
        ...
        if( SUCCEEDED( hr ) )
        {
            hr = pFactory->CreateInstance( NULL, __uuidof( IDiaDataSource ), (void**)ppSource );
            pFactory->Release();
        }
        else
        {
...
bool DumpMapToSrc(IDiaSession *pSession, DWORD dwRVA)
{
  ...
  for (; SUCCEEDED(pEnumStreams->Next(1, &pStream, &celt)) && (celt == 1); pStream = NULL) {
    ...
    if (bstrName && wcscmp(bstrName, L"OMAPTO") == 0) {
      ...
      PRINTF(L"image rva = %08X ==> source rva = %08X\n\nRelated OMAP entries:\n", dwRVA, dwRVATo);
...
bool DumpMapFromSrc(IDiaSession *pSession, DWORD dwRVA)
{
  ...
  for (; SUCCEEDED(pEnumStreams->Next(1, &pStream, &celt)) && (celt == 1); pStream = NULL) {
    ...
    if (bstrName && wcscmp(bstrName, L"OMAPFROM") == 0) {
      ...
      PRINTF(L"source rva = %08X ==> image rva = %08X\n\nRelated OMAP entries:\n", dwRVA, dwRVATo);
